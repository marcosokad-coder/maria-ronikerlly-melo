...
export function BlogCard({post}:{post:BlogPost}){return <Reveal className="blog-card-reveal"><Link className="blog-card" href={`/blog/${post.slug}`}><div className="blog-image"><Image src={post.coverImage} alt={post.coverAlt} fill sizes="(max-width: 700px) 100vw, 33vw"/></div><span className="eyebrow">{post.category}</span><h3>{post.title}</h3><p>{post.excerpt}</p><span className="text-link">Ler conteúdo <ArrowUpRight size={17}/></span></Link></Reveal>}
